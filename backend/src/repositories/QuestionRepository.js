...
class QuestionRepository {
  ...
  async findById(id) {
    ...
    if (element != null && element.length > 0) return element[0];
    return element;
  }
  ...
  async countByCategories(category_ids) {
    let convertedArray = category_ids.map((s) => ObjectId(s));
    let [{ count }] = await this.model.aggregate([
      {
        $match: {
          category_id: { $in: convertedArray },
        },
      },
      {
        $count: "count",
